/**
 * @file ThreeFiberLightRay.test.jsx
...
describe('components', () => {
  describe('ThreeFiberLightRay', () => {
    it('should mount with props', function () {
      const wrap = mount(<ThreeFiberLightRay>Hello World</ThreeFiberLightRay>)
    ...
    it('should render as type and with content', function () {
      const wrap = render(<ThreeFiberLightRay>Hello World</ThreeFiberLightRay>)

      expect(wrap[0].type).toEqual('tag')
      expect(wrap[0].name).toEqual('div')
      // contain styles
      expect(wrap[0].attribs.class).toContain('three_fiber_light_ray')
